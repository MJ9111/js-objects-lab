...
game.catchPokemon = function (pokemonObj) { this.party.push(pokemonObj); const pokeballIndex = this.items.findIndex((item) => item.name === "pokeball"); this.items[pokeballIndex].quantity--; };

game.catchPokemon(pokemon[40]); console.log(game.items);
...
game.gymStatus = function () { const gymTally = { completed: 0, incomplete: 0 }; this.gyms.forEach((gym) => { if (gym.completed) { gymTally.completed++; } else { gymTally.incomplete++; } }); console.log(gymTally); };
...
game.partyCount = function () { return this.party.length; };
